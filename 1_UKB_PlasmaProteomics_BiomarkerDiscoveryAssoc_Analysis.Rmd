...
```{r}
box_plotter <- function(input_tb, pheno, output_path, grouping_var='hcm', xunits='NPX'){
...
This is mini post-publication test for ANGPT2/ANGPT1 ratio as per Yingjie.

```{r}
test <- mutate(ukb_pp_hcm_cov,angpt2_1_ratio = angpt2/angpt1) %>% 
  mutate(ifelse(is.infinite(angpt2_1_ratio),NA,angpt2_1_ratio))
angpt2_1_check <- map(c('angpt2','angpt1','angpt2_1_ratio','ntprobnp'), ~limma_dpe(test, ., covar=covars)) %>% bind_rows()
```
...
multivariable_cox <- function(input_tb, marginal_predictor, covariates, status, outcome_var, outcome_levels, print_summary=T,return_predictor_results_only=T, return_model=F){ #Return only the coefficient and p-values for the marginal_predictor
  ...
  #First I run a linear regression between covariates and marginal predictor to estimate the R-squared for this protein (i.e. variance inflation factor)
  # 1. Construct the formula: Protein ~ Covariate1 + Covariate2 + ...
  x <- str_c(covariates, collapse='+')
  formula_str <- paste(marginal_predictor, '~', x)
  formula <- as.formula(formula_str)
  lm_model <- lm(formula, data = input_tb, na.action=na.omit)
  pp_cov_r_squared <- summary(lm_model)$r.squared

  #Run the Cox regression itself
  ...
  res<-bind_cols(vars,beta, se_beta, HR, HR.confint.lower, HR.confint.upper, p.value, z.value,pp_cov_r_squared) %>%
  ...
  names(res)<-c('var',"beta",'se_beta', "HR", "Lower_95CI_HR","Upper_95CI_HR", 
            "p.value", 'z.value', 'pp_cov_Rsq','predictor','outcome')
